...
from ezgatr.nn import EquiLinear, EquiRMSNorm, EquiLinearASL, EquiRMSNormASL
...
class MVOnlyGATrEmbeddingASL(nn.Module):
    r"""Embedding layer to project input number of channels to hidden channels.
...
class MVOnlyGATrBilinearASL(nn.Module):
    r"""Implements the geometric bilinear sub-layer of the geometric MLP.
    ...
    config: MVOnlyGATrConfig
    proj_bil: EquiLinearASL
    proj_out: EquiLinearASL
    ...
    def __init__(self, config: MVOnlyGATrConfig) -> None:
        ...
        self.proj_bil = EquiLinearASL(
            config.size_channels_hidden, config.size_channels_intermediate * 4
        )
        self.proj_out = EquiLinearASL(
            config.size_channels_intermediate * 2, config.size_channels_hidden
        )
    ...
    def forward(
        ...
        x = torch.cat([geometric_product_cpp(lg, rg), equi_join_cpp(lj, rj, reference)], dim=-2)
        return self.proj_out(x)
...
class MVOnlyGATrMLPASL(nn.Module):
    r"""Geometric MLP block without scaler channels.
    ...
    config: MVOnlyGATrConfig
    layer_norm: EquiRMSNormASL
    equi_bil: MVOnlyGATrBilinearASL
    proj_out: EquiLinearASL
    ...
    def __init__(self, config: MVOnlyGATrConfig) -> None:
        ...
        self.layer_norm = EquiRMSNormASL(
            config.size_channels_hidden,
            eps=config.norm_eps,
            channelwise_rescale=config.norm_channelwise_rescale,
        )
        self.equi_bil = MVOnlyGATrBilinearASL(config)
        self.proj_out = EquiLinearASL(
            config.size_channels_hidden, config.size_channels_hidden
        )
    ...
    def forward(
        ...
        x = self.layer_norm(x)
        x = self.equi_bil(x, reference)
        x = self.proj_out(scaler_gated_gelu_cpp(x, self.config.gelu_approximate))
...
class MVOnlyGATrAttentionASL(nn.Module):
    r"""Geometric attention block without scaler channels.
...
class MVOnlyGATrBlockASL(nn.Module):
    r"""GATr block without scaler channels.
    ...
    config: MVOnlyGATrConfig
    layer_id: int
    mlp: MVOnlyGATrMLPASL
    attn: MVOnlyGATrAttentionASL
    ...
    def __init__(self, config: MVOnlyGATrConfig, layer_id: int) -> None:
        ...
        self.mlp = MVOnlyGATrMLPASL(config)
        self.attn = MVOnlyGATrAttentionASL(config)
...
class MVOnlyGATrModelASL(nn.Module):
    r"""Multi-Vector only GATr model.
    ...
    config: MVOnlyGATrConfig
    embedding: MVOnlyGATrEmbeddingASL
    blocks: nn.ModuleList
    head: EquiLinearASL
    ...
    def __init__(self, config: MVOnlyGATrConfig) -> None:
        ...
        self.embedding = MVOnlyGATrEmbeddingASL(config)
        self.blocks = nn.ModuleList(
            MVOnlyGATrBlockASL(config, i) for i in range(config.num_layers)
        )
        self.head = EquiLinearASL(config.size_channels_hidden, config.size_channels_out)
        self.apply(self._init_params)
    ...
    def _init_params(self, module: nn.Module):
        ...
        module : nn.Module
            Module to initialize.
        """
        if isinstance(module, EquiLinearASL):
            nn.init.kaiming_uniform_(module.weight, nonlinearity="relu")
            module.weight.data /= math.sqrt(self.config.num_layers)
            if module.bias is not None:
                nn.init.zeros_(module.bias)
