...
# ---------------------------------------------------------------------------
# v4 - register-resident SoA (AVX2, float32). 8-multivector tiles transposed and
# computed in registers (no stack buffers). The 8-MV tile means correctness must
# be checked at sizes that span the tile boundary (N = 8k) AND the scalar
# remainder (N % 8 != 0); a random small batch_shape would not reliably do that.
# ---------------------------------------------------------------------------
# 1, 15 -> remainder; 8, 16, 32, 48 -> whole tiles; 15, 17, 31, 33, 127 -> tile + remainder.
_V4_TILE_SIZES = [1, 7, 8, 9, 15, 16, 17, 31, 32, 33, 48, 127, 128, 257, 2048]
...
def test_geometric_product_v4_float32(n):
    ...
    # vs the Python reference (the ground truth) and vs v3 (same math, different SIMD).
...
def test_equi_join_v4_float32(n):
    ...
    # domain, so it must be checked explicitly).
